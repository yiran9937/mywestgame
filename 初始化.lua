...
function 取开天辟地(攻击方)
    ...
    if r then
        -- 1. 获取攻击方的最大法力上限（增加安全保底）
        local 最大魔法 = 攻击方.最大魔法 or 0

        -- 2. 参考物理内丹公式，计算法力上限带来的系数加成
        -- （可根据服内平衡需求调整 0.00001 或 1.57 等参数）
        local 法力加成 = 0.000002 * math.pow(最大魔法, 1.57)
        ...
        -- print(string.format("[调试]: ndjl:%.2f", ndjl))
    end
    return ndjl
end
...
function 取鬼法伤害(攻击方,挨打方,伤害)
    if 攻击方.ev:忽视抗鬼火(攻击方,挨打方,伤害) then
        伤害 = 伤害 * (1 + (攻击方.加强鬼火 + 攻击方.忽视抗鬼火) * 0.01)
    else
...
function 取人法几率(类型, 几率, 攻击方, 挨打方)
    ...
    -- 2. 引入克与无属性伤害加成
    几率 = 强克伤害加成(攻击方, 挨打方, 几率)
    local 修改后几率 = 几率

    -- 3. 计算浮动与保底
    几率 = 几率 * math.random(90, 100) * 0.01
    if 几率 <= 0 then
        几率 = 1 -- 最低保留1%的概率
    end
    local 最终几率 = 几率
    ...
    -- 一行打印调试信息
    -- print(string.format("[%s调试]: 攻击方:%s,挨打方:%s, 原始几率: %.2f | 修改后几率: %.2f | 最终几率: %.2f", 类型, 攻击方.名称, 挨打方.名称, 原始几率, 修改后几率, 最终几率))
    return 几率
end
