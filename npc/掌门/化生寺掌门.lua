...
local _smjn = { '反间之计', '情真意切', '谗言相加', '借刀杀人', '失心狂乱' }
local _消耗 = {
    2000,
    6000,
    12500,
...
function NPC:NPC对话(玩家, i)
    if 玩家.种族 == 1 and 玩家.性别 == 1 then
        local r = 玩家:取任务('日常_师门任务')
        if r and r.门派 == 'hs' then
            return 对话[2]
        end
        return 对话[1]
    end
...
function NPC:领取师门任务(玩家, jn, 序号)
    ...
    local r = 生成任务 { 名称 = '日常_师门任务', 分类 = 分类,技能 = jn, 序号 = 序号,门派 = 'hs' }
...
function NPC:NPC给予(玩家, cash, items)
    local r = 玩家:取任务('日常_师门任务')
    if r and r.门派 == 'hs' and items[1] then --
