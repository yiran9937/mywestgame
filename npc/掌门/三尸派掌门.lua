...
local _smjn = { '吸血水蛭', '六翅毒蝉', '啮骨抽髓', '血煞之蛊', '吸星大法' }
local _消耗 = {
    2000,
    6000,
    12500,
...
function NPC:NPC对话(玩家, i)
    if 玩家.种族 == 4 and 玩家.性别 == 1 then
        local r = 玩家:取任务('日常_师门任务')
        if r and r.门派 == 'ss' then
            return 对话[2]
        end
        return 对话[1]
    end
...
function NPC:领取师门任务(玩家, jn, 序号)
    ...
    local r = 生成任务 { 名称 = '日常_师门任务', 分类 = 分类, 技能 = jn, 序号 = 序号, 门派 = 'ss' }
...
function NPC:NPC给予(玩家, cash, items)
    local r = 玩家:取任务('日常_师门任务')
    if r and r.门派 == 'ss' and items[1] then --
