        ...
        if r then
            ...
            if 物品 then
                if 对象:添加物品({ 物品 }) then
                    self.数量 = self.数量 - 1
                    if r.广播 then
                        对象:发送系统("#C%s使用藏宝图，一铲子下去，发现一个#G#m(%s)[%s]#m#n#89", 对象.名称, r.nid, r.名称)
                    end
                end
            end
        else
...
local _地图 = {
    1208 --, 1217, 101300, 101299, 101295
...
function 物品:刷出妖王(玩家)
    ...
    for _ = 1, 3, 1 do
        ...
        map:添加NPC {
            名称 = "游荡的千年妖魔",
            外形 = 2018,
            脚本 = 'scripts/npc/挖宝妖魔.lua',
            时间 = self.时间,
            X = X,
            Y = Y,
        }
    end
    玩家:发送系统("#R%s#W挖宝时不小心打开的高僧封印的妖魔箱子，一群妖魔冲天而出，逃到了#Y%s。"
    , 玩家.名称, map.名称)
end
        ...
        if map and self.地图数据.id == map.id then
            if math.abs(self.地图数据.x - 对象.X) < 5 and math.abs(self.地图数据.y - 对象.Y) < 5 then
                if math.random(100) < 20 then
                    self:刷出妖王(对象)
                     对象:提示窗口('#R你打开宝箱，一片黑影闪过，从宝箱里窜出一个恐怖的魔王，狂笑一声后消失的无影无踪...')
                    self.数量 = self.数量 - 1
                else
                    self:掉落包(对象)
                end
