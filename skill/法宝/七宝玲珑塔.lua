...
function 法术:法术取描述()
    ...
    local bl = 16 + math.floor(self.等级 * 0.2) * 0.01
    return string.format("%s回合内将遭受的%s%%法力伤害转化为气血伤害返还给对手", h, bl)
end
...
function BUFF:BUFF魔法伤害(攻击方, 挨打方) --75 满级
    if 攻击方.魔法伤害 > 0 then
        local bl = 16 + math.floor(self.等级 * 0.2) * 0.01
        local 气血伤害 = math.floor(攻击方.魔法伤害 * bl * 0.01)
        local 魔法伤害 = math.floor(攻击方.魔法伤害 - 气血伤害)
        攻击方:减少气血(气血伤害)
        return 魔法伤害
    end
end
