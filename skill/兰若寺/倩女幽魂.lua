...
function 法术:法术取BUFF效果_我方(攻击方, 挨打方)
    ...
    效果.抗震慑 = math.floor(13 * (1 + 攻击方.加强魅惑 * 0.01) * 100) * 0.01
    效果.抗遗忘 = math.floor(12 * (1 + 攻击方.加强魅惑 * 0.01) * 100) * 0.01
...
function 法术:法术取BUFF效果_敌方(攻击方, 挨打方)
    local 效果 = { 抗震慑 = 0, 抗遗忘 = 0, 抗鬼火 = 0, 抗三尸虫 = 0, 抗毒伤害 = 0 }
    效果.抗震慑 = math.floor(13 * (1 + 攻击方.加强魅惑 * 0.01) * 50) * 0.01
    效果.抗遗忘 = math.floor(12 * (1 + 攻击方.加强魅惑 * 0.01) * 50) * 0.01
...
function 法术:法术取回合()
    if self.熟练度 >= 18300 then
        return 7
    elseif self.熟练度 >= 14300 then
        return 6
...
function 法术:取目标数() --这个是按熟练度区分的
    if self.熟练度 >= 12000 then
        return 7
    elseif self.熟练度 >= 9600 then
        return 6
    elseif self.熟练度 >= 4600 then
        return 5
    elseif self.熟练度 >= 2200 then
        return 4
    else
        return 3
    end
...
function 法术:取鬼火()
    return math.floor((8 + self.熟练度 ^ 0.31787) * 100) * 0.01
end

function 法术:取三尸()
    return math.floor(5287 + self.熟练度 ^ 0.9019)
end

function 法术:取毒伤害()
    return math.floor((5287 + self.熟练度 ^ 0.9019) * 2)
end

--按描述应该是 给己方使用的话 就全是给自己方  而不应该带对方是吧 是的 五法给自己人用是加鬼族抗性  给对方用是减对方抗性
function 法术:法术取描述()
    return string.format(
        '对己方使用增加目标13%%抗震慑，12%%抗遗忘,#R%s%%#G抗鬼火,#R%s#G抗三尸虫,#R%s#G抗毒伤害,对敌方使用时降低对应抗性,效果减半，目标人数#R%s#G人，持续#R%s#G个回合。'
        , self:取鬼火(), self:取三尸(), self:取毒伤害(), self:法术取目标数(), self:法术取回合())
end
