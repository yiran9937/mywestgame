...
function 法术:法术取BUFF效果_我方(攻击方, 挨打方)
    ...
    效果.抗震慑 = math.floor(15 * (1 + 攻击方.加强魅惑 * 0.01) * 100) * 0.01
...
function 法术:法术取BUFF效果_敌方(攻击方, 挨打方)
    local 效果 = { 抗震慑 = 0, 抗遗忘 = 0, 抗鬼火 = 0, 抗三尸虫 = 0, 抗毒伤害 = 0 }
    效果.抗震慑 = math.floor(15 * (1 + 攻击方.加强魅惑 * 0.01) * 50) * 0.01
...
function 法术:取鬼火()
    return math.floor((5 + self.熟练度 ^ 0.29583) * 100) * 0.01 --25
end

function 法术:取三尸()
    return math.floor(4287 + self.熟练度 ^ 0.93024) --16622
end

function 法术:取毒伤害()
    return math.floor((4287 + self.熟练度 ^ 0.93024) * 2) --33244
end

function 法术:法术取描述()
    return string.format('对己方使用增加目标15%%抗震慑，12%%抗遗忘,#R%s%%#G抗鬼火,#R%s#G抗三尸虫,#R%s#G抗毒伤害,对敌方使用时降低对应抗性,效果减半，目标人数1人，持续#R%s#G个回合。'
