...
-- @Last Modified time  : 2023-06-24 01:41:33
...
local _描述 = '去洛阳城%s附近教训恶酒鬼#R强盗#R#u#[%s|%s|%s|$恶酒鬼#]#u#W。今天你已经帮了本客栈%s次。距离本次任务结束时间还有%s分钟，加油！'
...
function 任务:添加任务(玩家) --1寻药
    ...
    self.NPC =
        map:添加NPC {
            名称 = "恶酒鬼",
            外形 = 3,
            脚本 = 'scripts/task/宝图任务/宝图_送酒.lua',
            时长 = 1800,
            任务类型 = "宝图任务",
            X = X,
            Y = Y,
    ...
    self.mapid = map.id
    玩家:添加任务(self)
    return '上次有坛客人的酒被#Y恶酒鬼' .. self.位置 .. '#W在半道拦下给喝掉了，是可忍，孰不可忍，少侠速去给他点颜色看看。'
end
...
--==================================================
local 对话 = [[不错酒就是我喝的，大爷我就喜欢喝酒，喝自己本事搞来的酒。
menu
1|大胆强盗，看打！
99|走你，我不认识你
]]
...
function 任务:NPC菜单(玩家, i)
    if self.任务类型 == "宝图任务" then
        ...
        if r and self.nid == r.NPC then
            if i == "1" then
                玩家:进入战斗('scripts/task/宝图任务/宝图_送酒.lua', self)
            end
        else
            return "我认识你么？"
        end
...
function 任务:任务攻击事件(玩家, NPC)
    if NPC.任务类型 == "宝图任务" then
        if NPC.nid == self.NPC then
            玩家:进入战斗('scripts/task/宝图任务/宝图_送酒.lua', NPC)
        else
            return "我认识你么？"
        end
    end
