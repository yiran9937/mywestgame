...
-- @Last Modified time  : 2023-06-24 01:35:35
...
local _描述 = '#G#u#[%s|%s|%s|$强盗#]#u#W在大唐边境的#Y%s#W附近，请前去解救被打劫的店小二。今天你已经帮了本客栈%s次。距离本次任务结束时间还有%s分钟，加油！'



function 任务:任务取详情(玩家)
    if self.时间 then
        return string.format(_描述, self.mapid, self.x, self.y, self.位置, self.环数, (self.时间 - os.time()) //
            60)
    end
end
...
function 任务:添加任务(玩家) --1寻药
    ...
    self.NPC =
        map:添加NPC {
            名称 = "强盗",
            外形 = 2044,
            脚本 = 'scripts/task/宝图任务/宝图_解救小二.lua',
            时长 = 1800,
            任务类型 = "宝图任务",
            X = X,
            Y = Y,
    ...
    self.mapid = map.id
    玩家:添加任务(self)
    return '店小二前阵子在#Y大唐边境' .. self.位置 .. '遭到了打劫，大侠你有刷子，你去解救下。'
end

function 任务:掉落包(玩家) --1寻药
    local 经验 = math.floor(59884 * (1 + self.环数 * 0.35)) * 3 --1154万经验
...
function 任务:NPC菜单(玩家, i)
    if self.任务类型 == "宝图任务" then
        ...
        if r and self.nid == r.NPC then
            if i == "1" then
                玩家:进入战斗('scripts/task/宝图任务/宝图_解救小二.lua', self)
            end
        else
            return "我认识你么？"
        end
...
function 任务:任务攻击事件(玩家, NPC)
    if NPC.任务类型 == "宝图任务" then
        if NPC.nid == self.NPC then
            玩家:进入战斗('scripts/task/宝图任务/宝图_解救小二.lua', NPC)
        else
            return "我认识你么？"
        end
    end
