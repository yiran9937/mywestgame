...
local _描述 = '速去#G洛阳城%s#W找#Y#R#u#[%s|%s|%s|$狂妄的书生#]#u#W切磋切磋，是骡子是马就看你本事了#89。今天你已经帮了本客栈%s次。距离本次任务结束时间还有%s分钟，加油！'
...
function 任务:添加任务(玩家) --1寻药
    ...
    self.NPC =
        map:添加NPC {
            名称 = "狂妄的书生",
            外形 = 50,
            脚本 = 'scripts/task/宝图任务/宝图_狂妄书生.lua',
            时常 = 1800,
            任务类型 = "宝图任务",
            X = X,
            Y = Y,
    ...
    self.mapid = map.id
    玩家:添加任务(self)
    return '前阵子有个#Y狂妄的书生#W在本店放下狠话，说本店的客人没有一个可以干过他，我看你有两把刷子，去找他切磋切磋下，杀杀他的狂妄。'
end
...
function 任务:NPC菜单(玩家, i)
    if self.任务类型 == "宝图任务" then
        ...
        if r and self.nid == r.NPC then
            if i == "1" then
                玩家:进入战斗('scripts/task/宝图任务/宝图_狂妄书生.lua', self)
            end
        else
            return "我认识你么？"
        end
...
function 任务:任务攻击事件(玩家, NPC)
    if NPC.任务类型 == "宝图任务" then
        if NPC.nid == self.NPC then
            玩家:进入战斗('scripts/task/宝图任务/宝图_狂妄书生.lua', NPC)
        else
            return "我认识你么？"
        end
    end
