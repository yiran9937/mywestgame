...
local _描述 = '#W听说%s经常有骚扰路人的妖怪,你去%s把它们干掉。（遇敌）#W(当前第#R%s#W次，剩余#R%d#W分钟)'
function 任务:任务取详情(玩家)
    if self.时间 then
        if not self.击杀 then
            return string.format(_描述, self.map, self.map2, self.环数,
                (self.时间 - os.time()) // 60)
        else
            return "快去找帮派总管复命吧！"
        end
...
function 任务:添加任务(玩家)
    ...
    self.环数 = 玩家.其它.帮派环数
    local t = _地图[math.random(#_地图)]
    self.map = t[1]
    self.map2 = t[2]
    self.mapid = t[3]
    self.时间 = os.time() + 1800
    玩家:添加任务(self)
    self.最后对话 = string.format("#W听说%s经常有骚扰路人的妖怪,你去%s把它们干掉。", self.map,
        self.map2)
    return true
end
...
function 任务:完成击杀(玩家)
    ...
    local tx = self.map == "龙窟" and 2070 or 2071

    玩家:最后对话("大侠我错了，我再也不敢了。", tx)
end
...
function 任务:任务触发暗雷前(玩家, mapid) --1寻药
    if mapid == self.mapid and not self.击杀 then
        玩家.是否移动 = false
        玩家:进入战斗('scripts/task/帮派任务/帮派任务_妖怪.lua', self.map)
        return false
    end
end
