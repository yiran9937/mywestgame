...
--你得去整一组号 然后剧情都到 这个剧情，一点不要往下做 然后存一份 这个数据档  得看看是那里没同步
--现有的都做完了吧，技能哪个你测试慢慢改吧，我让我对象再弄个玉你拍？
--  这两天给主要的东西 弄完 你继续搞技能  我开始开服回血   嗯 那就先拍了剩下的玉，这两天给你改这些乱七八糟的东西，你去弄组号吧 弄好了 你就发 要快新玉 看看 我在的花就远程你 
...
function 任务:任务攻击事件(玩家, NPC)
    ...
    elseif NPC.名称 == '玉皇大帝' then
        if self.进度 == 4 then
            local r = 玩家:进入战斗('scripts/task/称谓剧情/称谓9_抹杀孙悟空.lua', 2)
            if r then
                for _, v in 玩家:遍历队伍(玩家) do
                    local 任务 = v:取任务(self.名称)
                    if 任务 and 任务.进度 == 4 then
                        任务.进度 = 5
                        任务.对话进度 = 任务.对话进度 + 1
                        NPC.台词, NPC.头像, NPC.结束 = 任务:取对话(v)
                        v:最后对话(NPC.台词, NPC.头像, NPC.结束)
